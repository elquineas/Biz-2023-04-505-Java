...
public class ExecB {
	public static void main(String[] args) {
		 ...
		 * 생성자 method( ScoreDto() )사용하여 객체를 초기화(new) 한다.
		 * 
		 * scoreDto 객체를 초기화하여 사용할 준비가 되면 인스턴스가 되었다
		 * 라고 표현한다.
		 * 
		 * scoreDto 객체를 초기화 하는 순간
		 * ScoreDto 클래스에서 선언한 변수들(여기서는 9가지)을 모두
		 * 사용할 수 있도록 동시에 초기화가 된다.
		 */
		ScoreDto scoreDto = new ScoreDto();
		// 초기화된 scoreDto에 포함된 변수들의 값을 다시 세팅
		scoreDto.kor	= (int)(Math.random()*50)+51;
		scoreDto.eng	= (int)(Math.random()*50)+51;
		scoreDto.meth	= (int)(Math.random()*50)+51;
		scoreDto.hist	= (int)(Math.random()*50)+51;
		scoreDto.music	= (int)(Math.random()*50)+51; 
		scoreDto.art	= (int)(Math.random()*50)+51;
		scoreDto.sport	= (int)(Math.random()*50)+51;
		scoreDto.stNum	= "0001";
		scoreDto.stName	= "홍길동";
